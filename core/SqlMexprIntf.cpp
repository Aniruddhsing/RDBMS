...
#include "/home/aniruddh/DBMS/SqlParser/ParserExport.h"
#include "/home/aniruddh/MathExpressionParser/Course/MexprTree.h"
#include "/home/aniruddh/MathExpressionParser/Course/Dtype.h"
#include "qep.h"
#include "sql_name.h"
...
sql_exptree_t *sql_create_exp_tree_for_column(const char *col_name) {
    if (!col_name) return NULL;
    sql_exptree_t *sql_exptree = (sql_exptree_t *)calloc(1, sizeof(sql_exptree_t));
    if (!sql_exptree) return NULL;
    // Build a postfix array with a single variable token
    lex_data_t **postfix = (lex_data_t **)calloc(1, sizeof(lex_data_t *));
    if (!postfix) {
        free(sql_exptree);
        return NULL;
    }
    postfix[0] = (lex_data_t *)calloc(1, sizeof(lex_data_t));
    if (!postfix[0]) {
        free(postfix);
        free(sql_exptree);
        return NULL;
    }
    postfix[0]->token_code = SQL_IDENTIFIER;
    postfix[0]->token_len = strlen(col_name);
    postfix[0]->token_val = strdup(col_name);
    if (!postfix[0]->token_val) {
        free(postfix[0]);
        free(postfix);
        free(sql_exptree);
        return NULL;
    }
    sql_exptree->tree = new MexprTree(postfix, 1);
    // Free postfix array and its contents
    free(postfix[0]->token_val);
    free(postfix[0]);
    free(postfix);
    if (!sql_exptree->tree) {
        free(sql_exptree);
